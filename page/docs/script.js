...
// 🧩 Gabungkan params dari path dan dari JSON
const paramsFromPath = new URLSearchParams(apiData.path.split("?")[1] || "")
const pathParams = Array.from(paramsFromPath.keys())

// Ambil param tambahan dari settings.json
const jsonParams = apiData.params ? Object.keys(apiData.params) : []

// Gabungkan dua sumber dan hapus duplikat
const paramKeys = [...new Set([...pathParams, ...jsonParams])]

let paramContainer // supaya bisa dipakai di bawah (innerDesc, append, dsb)
...
if (paramKeys.length > 0) {
  ...
  formTitle.innerHTML =
    '<i class="fas fa-sliders-h me-2" aria-hidden="true"></i> Parameters'
  ...
  paramKeys.forEach((paramKey) => {
    ...
    paramLabelContainer.className = "param-label-container"

    const paramLabel = document.createElement("label")
    paramLabel.className = "form-label"
    paramLabel.textContent = paramKey
    paramLabel.htmlFor = `param-${paramKey}`

    const desc =
      apiData.params?.[paramKey] || "Enter value for this parameter"

    paramLabelContainer.appendChild(paramLabel)
    paramGroup.appendChild(paramLabelContainer)

    const paramInput = document.createElement("input")
    paramInput.type = "text"
    paramInput.name = paramKey
    paramInput.id = `param-${paramKey}`
    paramInput.className = "form-control"
    paramInput.placeholder = desc
...
// 🧩 Jika ada paramContainer
if (paramContainer) {
  DOM.modal.queryInputContainer.appendChild(paramContainer)
} 
// 🔑 Jika tidak ada params tapi require API key
  ...
  formTitle.className = "param-form-title"
  formTitle.innerHTML =
    '<i class="fas fa-key me-2" aria-hidden="true"></i> API Key Required'
  paramContainer.appendChild(formTitle)
...
// ✅ Akhir: setup tombol dan tooltip (satu kali saja)
